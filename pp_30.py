...
def map_longest(text: str) -> tuple:
    ...
    '''
    split_text: list[str] = text.lower().split()
    tuple_text: list[tuple[str, int]] = list(
        map(lambda x: (x, len(x)), split_text))
    tuple_text.sort(key=lambda x: x[1], reverse=True)
    return tuple_text[0]
...
result: tuple[str, int] = map_longest(text)
print(
